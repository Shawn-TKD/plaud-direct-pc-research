...
class FastTransferSheet : BottomSheetDialogFragment() {
    ...
    private val wifiPermissionLauncher = registerForActivityResult(
        ...
        if (granted) {
            ...
            showError("需要附近 Wi-Fi 设备权限才能连接 NotePin 热点")
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.turnOnButton.setOnClickListener {
            if (WifiTransferPermission.isGranted(requireContext())) {
                beginTransfer()
            } else {
                wifiPermissionLauncher.launch(WifiTransferPermission.requiredPermission)
            }
        }
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                syncManager.state.collect { state ->
                    ...
                    when (state) {
                        ...
                        is SyncState.Failed -> showError(state.message)
                        else -> {}
                    }
                }
            }
